...
<div class="container py-4">
    <div class="mb-4">
        <h1 class="fw-bold mb-1" style="font-size: 1.65rem; color: #1e1e2d;">Nouveau devis</h1>
        <p class="text-muted mb-0" style="font-size: 0.95rem;">Remplissez les informations ci-dessous pour g&eacute;n&eacute;rer votre devis en PDF.</p>
    </div>
    ...
    <form action="{{ route('quote-tool.quote.store') }}" method="POST" id="quoteForm">
        @csrf
        <div class="row">
            {{-- LEFT COLUMN --}}
            <div class="col-lg-8">
                ...
                @if($hasSubscription ?? false)
                    <div class="credit-banner subscription">
                        <i class="bi bi-trophy" style="font-size: 1.2rem; color: #059669;"></i>
                        Inclus dans votre abonnement &mdash; G&eacute;n&eacute;ration illimit&eacute;e de devis
                    </div>
                @elseif($isFree)
                    <div class="credit-banner free">
                        <i class="bi bi-gift" style="font-size: 1.2rem;"></i>
                        Essai gratuit &mdash; Cr&eacute;ez votre premier devis gratuitement !
                    </div>
                @else
                ...
                <div class="card quote-card">
                    <div class="card-header">
                        <i class="bi bi-building"></i> Informations du client
                    </div>
                    <div class="card-body">
                        <div class="row g-3">
                            <div class="col-md-6">
                ...
                <div class="card quote-card">
                    <div class="card-header">
                        <i class="bi bi-list-check"></i> D&eacute;tails du devis
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            <label for="subject" class="form-label">Objet du devis <span class="text-danger">*</span></label>
                            <input type="text" class="form-control" id="subject" name="subject" value="{{ old('subject') }}" required placeholder="Ex : Prestation de conseil, R&eacute;novation salle de bain...">
                        </div>
                            ...
                            <table class="items-table" id="itemsTable">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="width: 20%;">Prix unitaire HT (&euro;)</th>
                                ...
                                <tbody id="itemsBody">
                                    <tr class="item-row">
                                        <td>
                                            <input type="text" class="form-control" name="items[0][description]" placeholder="Description du produit ou service" required>
                                        </td>
                                        <td>
                                            <input type="number" class="form-control item-qty" name="items[0][quantity]" value="1" min="0" step="any" oninput="recalculate()">
                                        </td>
                                        <td>
                                            <input type="number" class="form-control item-price" name="items[0][unit_price]" value="0" min="0" step="0.01" oninput="recalculate()">
                                        </td>
                                        <td class="row-total">0.00 &euro;</td>
                                        <td class="text-center">
                                            <button type="button" class="btn-remove-item" onclick="removeItem(this)" title="Supprimer la ligne">&times;</button>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                ...
                <div class="card quote-card">
                    <div class="card-header">
                        <i class="bi bi-journal-text"></i> Notes et conditions
                    </div>
                    <div class="card-body">
                        <div class="mb-3">
                            <label for="notes" class="form-label">Notes</label>
                            <textarea class="form-control" id="notes" name="notes" rows="3" placeholder="Informations compl&eacute;mentaires pour le client...">{{ old('notes') }}</textarea>
                        </div>
                        <div>
                            <label for="conditions" class="form-label">Conditions</label>
                            <textarea class="form-control" id="conditions" name="conditions" rows="3">{{ old('conditions', 'Devis valable 30 jours à compter de la date d\'émission. Paiement à réception de facture.') }}</textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-lg-4">
                <div class="sticky-sidebar">
                    ...
                    <div class="card quote-card">
                        ...
                        <div class="card-body">
                            ...
                            <div>
                                ...
                                <input type="date" class="form-control" id="valid_until" name="valid_until" value="{{ old('valid_until', \Carbon\Carbon::now()->addDays(30)->format('Y-m-d')) }}">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card quote-card totals-card">
                        <div class="card-header">
                            <i class="bi bi-calculator"></i> R&eacute;capitulatif
                        </div>
                        <div class="card-body">
                    ...
                    <button type="submit" class="btn btn-generate">
                        <i class="bi bi-file-earmark-pdf me-2"></i> G&eacute;n&eacute;rer mon devis PDF
                    </button>
...
<script>
    ...
    function addItem() {
        ...
        tr.innerHTML = `
            <td>
                <input type="text" class="form-control" name="items[${itemIndex}][description]" placeholder="Description du produit ou service" required>
            </td>
            <td>
                <input type="number" class="form-control item-qty" name="items[${itemIndex}][quantity]" value="1" min="0" step="any" oninput="recalculate()">
            </td>
            <td>
                <input type="number" class="form-control item-price" name="items[${itemIndex}][unit_price]" value="0" min="0" step="0.01" oninput="recalculate()">
            </td>
            <td class="row-total">0.00 &euro;</td>
            <td class="text-center">
                <button type="button" class="btn-remove-item" onclick="removeItem(this)" title="Supprimer la ligne">&times;</button>
    ...
    function recalculate() {
        ...
        document.getElementById('totalDisplay').textContent = totalTTC.toFixed(2) + ' \u20AC';
    }
